...
class CommentService {
    async commentAdd(comment, _id, animeId) {
        ...
        const commentCreate = await Comments.create({ comment: comment, idUser: userDto.id, login: userDto.login, animeId: animeId });
        return commentCreate;
    }
    async updateComment(comment, _id) {
        const userComment = await Comments.findOne({ _id });

        if(!userComment) {
            throw ApiError.BadRequest();
        }

        const updateComment = await Comments.findOneAndUpdate({ _id }, { comment: comment, timestamp: String(Date.now()), changed: true }, { new: true });
    ...
    async likeAdd(currentIdUser, idComment) {
        ...
        const comment = await Comments.findById({ _id: idComment });
        if(!comment) {
            throw ApiError.BadRequest();
        }
